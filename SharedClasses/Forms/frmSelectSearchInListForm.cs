    ...
    public partial class frmSelectSearchInListForm : Form
    {
        ...
        private Control CreateValueTextBox(ref sFildsDescription sfd, int y, int x, int w = 50, string prefix = "txt_v_")
        {
            ...
            if (sfd.mTOT == TypeOfType.DICTIONARY)
            {
                ...
                {
                    ReestrDictionaryWork.EnableAutoComlectToFizLic(temp);
                }
                else if (sfd.mDictionareTableName == "jur_osoby")
                {
                    ReestrDictionaryWork.EnableAutoComlectToJurLic(temp);
                }
                else if (sfd.mDictionareTableName == "Tip_Doc")
                {
                    ReestrDictionaryWork.EnableAutoComlectToTip_Doc(temp);
                }
                else
                {
                    temp.Enabled = false;
            ...
            if (txt != null && txt.Tag != null)
            {
                ...
                if (sfd != null)
                {
                    ...
                    else if (sfd.mTOT == TypeOfType.DICTIONARY)
                    {
                        if (sfd.mDictionareTableName == "fizichni_osoby")
                        {
                            ReestrDictionaryWork.CheckValueIsContainsFizLic_SetError(txt, errorProvider, ref sfd.mDictionareValue);
                        }
                        else if (sfd.mDictionareTableName == "jur_osoby")
                        {
                            ReestrDictionaryWork.CheckValueIsContainsJurLic_SetError(txt, errorProvider, ref sfd.mDictionareValue);
                        }
                        else if (sfd.mDictionareTableName == "Tip_Doc")
                        {
                            ReestrDictionaryWork.CheckValueIsContainsTip_Doc_SetError(txt, errorProvider, ref sfd.mDictionareValue);
                        }
                        else
                        {
                        }
            ...
            if (txt != null && txt.Tag != null)
            {
                ...
                if (sfd != null)
                {
                    ...
                    else if (sfd.mTOT == TypeOfType.DICTIONARY)
                    {
                        if (sfd.mDictionareTableName == "fizichni_osoby")
                        {
                            e.Cancel = ReestrDictionaryWork.CheckValueIsContainsFizLic_SetError(txt, errorProvider, ref sfd.mDictionareValue);
                        }
                        else if (sfd.mDictionareTableName == "jur_osoby")
                        {
                            e.Cancel = ReestrDictionaryWork.CheckValueIsContainsJurLic_SetError(txt, errorProvider, ref sfd.mDictionareValue);
                        }
                        else if (sfd.mDictionareTableName == "Tip_Doc")
                        {
                            e.Cancel = ReestrDictionaryWork.CheckValueIsContainsTip_Doc_SetError(txt, errorProvider, ref sfd.mDictionareValue);
                        }
                        else
                        {
                        }
